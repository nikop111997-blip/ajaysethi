    ...
    <section id="bundles" className="py-24 bg-gray-50">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          <span className="text-[#ff6a3d] font-bold text-sm tracking-widest uppercase">Pricing</span>
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {BUNDLES.map((bundle) => (
            <div
              key={bundle.id}
              className={`relative bg-white rounded-lg p-8 md:p-10 border-2 transition-all cursor-pointer ${bundle.popular ? 'border-[#ff6a3d] shadow-2xl shadow-[#ff6a3d]/10 transform md:-translate-y-4' : 'border-gray-100 shadow-lg hover:border-gray-300'}`}
              onClick={() => openCartWithBundle(bundle)}
            >
              {bundle.popular && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#ff6a3d] text-white px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider">
                  Most Popular
                </div>
              )}
              ...
              <ul className="space-y-4 mb-8">
                {['Premium Hardcover Binding', 'Undated 90-Day Layout', 'Daily & Weekly Reviews', 'Free Shipping in India'].map((feature, i) => (
                  <li key={i} className="flex items-center gap-3 text-sm text-gray-600">
                    <CheckCircle2 className="w-5 h-5 text-[#ff6a3d]" /> {feature}
                  </li>
                ))}
                {bundle.popular && (
                  <li className="flex items-center gap-3 text-sm font-bold text-[#1a1a1a]">
                    <CheckCircle2 className="w-5 h-5 text-[#ff6a3d]" /> Full Year Supply (4 Books)
                  </li>
                )}
              </ul>

              <button
                className={`w-full py-4 rounded-md font-bold transition-colors ${bundle.popular ? 'bg-[#ff6a3d] hover:bg-[#e55a2d] text-white shadow-lg shadow-[#ff6a3d]/30' : 'bg-[#1a1a1a] hover:bg-black text-white'}`}
              >
                Select {bundle.name}
              </button>
            </div>
