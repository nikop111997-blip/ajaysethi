...
export default function WhoIsThisFor() {
  ...
  Start building <span className="text-[#ff6a3d]">scalable systems.</span>
          ...
          <p className="text-gray-600 text-md sm:text-lg leading-relaxed">
            ...
            <Link
              href="/business-audit"
              className="inline-flex items-center mt-4 sm:mt-0 -ml-11 sm:-ml-0 w-fit gap-4 bg-[#ff6a3d] text-[#ffffff] py-2 pl-6 pr-4 rounded-2xl text-base font-semibold hover:bg-[#ff7a53] transition-colors group"
            >
  ...
  <ArrowRight className="w-5 h-5 text-white absolute -translate-x-[150%] transition-transform duration-300 ease-in-out group-hover:translate-x-0" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {audiences.map((item, index) => (
            <div 
              key={index}
              className="bg-white rounded-[1rem] p-8 md:p-10 border border-gray-100 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] flex flex-col group hover:-translate-y-2 hover:shadow-xl hover:border-gray-200 transition-all duration-300 animate-audience relative overflow-hidden"
